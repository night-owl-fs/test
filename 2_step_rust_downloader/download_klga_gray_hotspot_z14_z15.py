...
def download(url: str, out: pathlib.Path, retries: int, timeout: int) -> tuple[bool, str]:
    ...
    for i in range(retries):
        try:
            req = urllib.request.Request(url, headers={"User-Agent": "klga-hotspot-fix/1.0"})
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Fix KLGA gray hotspot by replacing exact Z14/Z15 source ranges.")
    ap.add_argument("--url-template", required=True, help="Use placeholders: {airport} {z} {x} {y} {ext} {path}")
    ...
    # KLGA Z14 cone: Z14_fromZ9_r2_c2_baseX151_baseY193
    add_range(jobs, "KLGA", 14, 4832, 4863, 6176, 6207, root, args.include_pgw)

    # KLGA Z15 cone: Z15_fromZ9_r2_c2_baseX151_baseY193
    ...
    if fails:
        rep = root / "CODE" / "reports" / "klga_hotspot_download_failures.txt"
        rep.parent.mkdir(parents=True, exist_ok=True)
        rep.write_text("\n".join(fails) + "\n", encoding="utf-8")
        print(f"Failures: {rep}")
        return 2

    print("Download replace complete.")
    return 0
